...
template <typename Ty_, typename KernelFunc>
__host__ std::vector<Ty_> performOperator(const std::vector<Ty_> &a, Ty_ &&b,
                                          KernelFunc kernelFunction) {
  cudaError_t cudaStatus = cudaSuccess;
  ...
  dim3 blocksPerGrid(1024);
  dim3 threadsPerBlock(size / 1024);
  kernelFunction<<<blocksPerGrid, threadsPerBlock, 0, stream>>>(c, dev_a, b,
                                                                size);
...
performOperator<float, void (*)(float *, const float *, const float *,
                                uint32_t)>(const std::vector<float> &,
                                           const std::vector<float> &,
                                           void (*)(float *, const float *,
                                                    const float *, uint32_t));
template std::vector<double>
performOperator<double, void (*)(double *, const double *, const double *,
                                 uint32_t)>(const std::vector<double> &,
                                            const std::vector<double> &,
                                            void (*)(double *, const double *,
                                                     const double *, uint32_t));
template std::vector<float>
performOperator<float, void (*)(float *, const float *, const float &,
                                uint32_t)>(const std::vector<float> &, float &&,
                                           void (*)(float *, const float *,
                                                    const float &, uint32_t));
template std::vector<double>
performOperator<double, void (*)(double *, const double *, const double &,
